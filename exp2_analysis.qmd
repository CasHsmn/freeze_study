...
kable(tidy(risk_aov), caption = "ANOVA perceived risk by group")
kable(tidy(TukeyHSD(risk_aov)), caption = "Post-hoc Tukey comparisons")
```
## alternative k-means clustering
In k-means clustering, the cluster numbers are the same but they're otherwise very similar.
In this case, cluster 1 is the non-freezing group with the higher perceived risk.
...
# cluster summary
kable(aggregate(df_clust[, c("lik_1", "lik_2", "lik_3", "lik_4", "lik_5")], 
          by = list(cluster = df_clust$kmclust), FUN = mean), caption = "Mean behaviour likelihood scores by cluster", col.names = c("Cluster", "Freezing", "Discarding", "Cooking", "Fridge", "Share"), digits = 2)

# cluster sizes
kable(table(df_clust$kmclust), caption = "Cluster sizes", col.names = c("Cluster", "n"))
...
kable(tidy(risk_aov), caption = "ANOVA perceived risk by k-means clusters")
kable(tidy(TukeyHSD(risk_aovkm)), caption = "post-hoc tukey for k-means clusters")
...
# Step 2: Calculate averaged per_risk for each condition
df2 <- df2 %>%
  mutate(
    st_per_risk = rowMeans(cbind(st_per_risk1_1, st_per_risk2_1), na.rm = TRUE),
    s3_per_risk = rowMeans(cbind(s3_per_risk1_1, s3_per_risk2_1), na.rm = TRUE),
...
# Step 3: Pivot behavior variables to long format
beh_long <- df2 %>%
  pivot_longer(
    cols = c(st_beh_1, s3_beh_1, ft_beh_1, f3_beh_1),
    names_to = "condition",
    values_to = "beh_1"
  ) %>%
  mutate(condition = str_replace(condition, "_beh_1$", ""))  # Extract condition prefix (st, s3, ft, f3)

# Step 4: Pivot per_risk variables to long format
risk_long <- df2 %>%
  pivot_longer(
    cols = c(st_per_risk, s3_per_risk, ft_per_risk, f3_per_risk),
    names_to = "condition",
    values_to = "per_risk"
  ) %>%
  mutate(condition = str_replace(condition, "_per_risk$", ""))  # Extract condition prefix

# Step 5: Join behavior and per_risk data by id and condition
df_long <- beh_long %>%
  left_join(risk_long, by = c("id", "condition")) %>%
  mutate(
    expiry = recode(condition, "st" = 0, "s3" = 1, "ft" = 0, "f3" = 1),
    location = recode(condition, "st" = 0, "s3" = 0, "ft" = 1, "f3" = 1),
    expiry = factor(expiry, labels = c("today", "5 days")),
    location = factor(location, labels = c("shop", "fridge")),
    c_risk = per_risk - mean(per_risk, na.rm = TRUE)
  ) %>%
  select(id, expiry, location, beh_1, per_risk, c_risk)
```

## sample
```{r}
#| label: sample experiment 2
...
table1(~ gender + age + adu + chi + edu + inc, data = df2)
```
## descriptives
Still very little variety, though in this setup there is also quite clearly one correct answer, to freeze. Even then, there is still a difference between the expiration groups. Though people in both groups are very likely to freeze, so I am not sure if this difference would actually lead to any real differences in behaviour.
```{r}
df_long %>%
  mutate(expiry_location = paste(expiry, location, sep = "_")) %>%
  group_by(expiry_location) %>%
  summarise(mean_beh_1 = mean(beh_1, na.rm = TRUE), .groups = "drop") %>%
  ggplot(aes(x = expiry_location, y = mean_beh_1, fill = expiry_location)) +
  geom_col() +
...
## Inferential
```{r}
# location and expiry only
options(contrasts = c("contr.sum", "contr.poly"))
mmFreeze <- lme4::lmer(beh_1 ~ expiry * location + (1|id), data = df_long)
Anova(mmFreeze, test = "F", type = "3")

emFreeze <- emmeans(mmFreeze, pairwise ~ expiry * location)

# perceived risk effect
# path a
med_a <- lmer(per_risk ~ expiry * location + (1|id), data = df_long)
Anova(med_a, test = "F", type = 3)

# full model
med_full <- lmer(beh_1 ~ expiry * location + c_risk + (1|id), data = df_long)
Anova(med_full, test = "F", type = 3)
summary(med_full)

emFull <- emmeans(med_full, ~ expiry * c_risk)

#em <- emmeans(med, ~ expiry * per_risk)
#summary(em)
...
#emmip(em, ~ expiry * per_risk)

# mediation
mediator_model <- lme4::lmer(c_risk ~ expiry * location + (1|id), data = df_long)
outcome_model <- lme4::lmer(beh_1 ~ expiry * location + c_risk + (1|id), data = df_long)
med_results <- mediation::mediate(mediator_model, outcome_model, treat = "expiry", control.value = "today", treat.value = "5 days",mediator = "c_risk", sims = 1000)
...
#| eval: false
#| include: false
# chatgpt interactions package stuff to visualise
library(tidyverse)

# Calculate mean and SD
mean_pr <- mean(df_long$per_risk, na.rm = TRUE)
sd_pr <- sd(df_long$per_risk, na.rm = TRUE)

# Create prediction grid
pred_data <- expand.grid(
  expiry = unique(df_long$expiry),
  location = unique(df_long$location),
  per_risk = c(mean_pr - sd_pr, mean_pr, mean_pr + sd_pr),
  id = NA  # Set to average participant
)

# Get predictions
pred_data$predicted_beh <- predict(outcome_model, newdata = pred_data, re.form = NA)

# Add labels
pred_data$per_risk_level <- factor(
  ifelse(pred_data$per_risk == mean_pr - sd_pr, "-1 SD",
         ifelse(pred_data$per_risk == mean_pr, "Mean", "+1 SD")),
  levels = c("-1 SD", "Mean", "+1 SD")
)

# Plot
ggplot(pred_data, aes(x = expiry, y = predicted_beh, color = location, group = location)) +
  geom_point(linewidth = 3) +
  geom_line() +
  facet_wrap(~ per_risk_level) +
  labs(
    title = "Predicted beh_1 by Expiry and Location at Different Levels of per_risk",
    x = "Expiry",
    y = "Predicted beh_1",
    color = "Location"
  ) +
...
pred_data <- expand.grid(
  expiry = unique(df_long$expiry),
  location = unique(df_long$location),
  per_risk = c(mean_pr - sd_pr, mean_pr + sd_pr)
)

pred_data$c_risk <- pred_data$per_risk - mean_pr
pred_data$predicted_beh <- predict(med_full, newdata = pred_data, re.form = NA)

pred_data$per_risk_level <- factor(
  ifelse(pred_data$per_risk == mean_pr - sd_pr, "Low per_risk (-1 SD)", "High per_risk (+1 SD)"),
  levels = c("Low per_risk (-1 SD)", "High per_risk (+1 SD)")
)

# Plot
ggplot(pred_data, aes(x = per_risk_level, y = predicted_beh, color = expiry, group = expiry)) +
  geom_line(linewidth = 1) +
  geom_point(size = 3) +
  theme_minimal()
```
```{r}
df_long <- df_long %>%
  mutate(per_risk_c = per_risk - mean(per_risk, na.rm = TRUE))


outcome_model <- lmer(beh_1 ~ expiry * per_risk + (1|id), data = df_long)

# Calculate mean and SD of per_risk
mean_pr <- mean(df_long$per_risk, na.rm = TRUE)
sd_pr <- sd(df_long$per_risk, na.rm = TRUE)

# Create prediction data with per_risk levels and expiry conditions
pred_data <- expand.grid(
  per_risk = c(mean_pr - sd_pr, mean_pr, mean_pr + sd_pr),
  expiry = unique(df_long$expiry)
)

# Predict beh_1 at these values (population-level: no random effects)
pred_data$predicted_beh <- predict(outcome_model, newdata = pred_data, re.form = NA)

# Label per_risk levels for plotting
pred_data$per_risk_level <- factor(
  round(pred_data$per_risk, 2),
  levels = round(c(mean_pr - sd_pr, mean_pr, mean_pr + sd_pr), 2),
...
ggplot(pred_data, aes(x = per_risk_level, y = predicted_beh, color = expiry, group = expiry)) +
  ...
  geom_point(size = 3) +
  labs(
    title = "Predicted beh_1 by Perceived Risk and Expiry",
    x = "Perceived Risk",
    y = "Predicted beh_1",
    color = "Expiry"
  ) +
...
```{r}
#| label: analyse within-subject variability

within_person_variance <- df_long %>%
  group_by(id) %>%
  summarise(
    mean_per_risk = mean(per_risk, na.rm = TRUE),
    sd_per_risk = sd(per_risk, na.rm = TRUE),
    range_per_risk = max(per_risk, na.rm = TRUE) - min(per_risk, na.rm = TRUE),
    n_conditions = n()
  ) %>%
  ungroup()

# Summary statistics
summary(within_person_variance$sd_per_risk)
summary(within_person_variance$range_per_risk)

# Visualize distribution of within-person SD
ggplot(within_person_variance, aes(x = sd_per_risk)) +
  geom_histogram(bins = 30, fill = "steelblue", color = "black") +
  labs(
    title = "Distribution of Within-Person Variability in Perceived Risk",
    subtitle = "Red line indicates SD = 1 (low variability threshold)",
    x = "Standard Deviation of per_risk within person",
    y = "Count of Participants"
  ) +
  theme_minimal()

# Visualize distribution of within-person range
ggplot(within_person_variance, aes(x = range_per_risk)) +
  geom_histogram(bins = 20, fill = "darkorange", color = "black") +
  labs(
    title = "Distribution of Within-Person Range in Perceived Risk",
    subtitle = "Red line indicates range = 2 (low variability threshold)",
    x = "Range of per_risk within person (max - min)",
    y = "Count of Participants"
  ) +
  theme_minimal()
...
df_long %>%
  group_by(expiry, location) %>%
  summarise(
    mean_risk = mean(per_risk, na.rm = TRUE),
    sd_risk = sd(per_risk, na.rm = TRUE),
    n = n()
  )

# Visualization
ggplot(df_long, aes(x = interaction(expiry, location), y = per_risk, fill = expiry)) +
  geom_boxplot() +
  labs(
    title = "Perceived Risk by Condition",
    x = "Condition (Expiry × Location)",
    y = "Perceived Risk"
  ) +
  theme_minimal()
```
